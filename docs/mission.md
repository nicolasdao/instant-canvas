...
2. **Lean context over completeness** — the skill is large (a full runtime, 17 chart kinds, 16 field types), but the agent's context window is sacred. Progressive disclosure everywhere: a ~4 KB lean index first, one exact schema on demand, never the full contract unless explicitly requested.
...
5. **Zero dependencies over feature velocity** — plain Node ≥ 20, two vendored browser files, no build step, no npm install. Every feature must earn its place without a dependency.
